...
d3.json('http://127.0.0.1:5000/data').then(data => {
  ...
  var mcp32 = [];
  for (i=0; i < filtBear.length; i++) {
    mcp32.push([filtBear[i].lat, filtBear[i].lon])
  };
  
  var filtBear = data.filter(data => data.id === 281);
  var mcp281 = [];
  for (i=0; i < filtBear.length; i++) {
    mcp281.push([filtBear[i].lat, filtBear[i].lon])
  }; 
  console.log(mcp281)
    // var lat = filtBear.lat;
    // var lon = filtBear.lon;
...
var BearID_mcp281 =  L.polyline(mcp281 ,{color:"white"}).bindPopup("Bear 281");
